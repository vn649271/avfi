    ...
    farmsConfig.map(async (farmConfig) => {
      ...
      if(farmConfig.isTokenOnly){
        ...
        if(tokenAmount.comparedTo(0) > 0){
          ...
          if(farmConfig.pid === 5)
          {
            tokenPriceVsQuote = tokenPriceVsQuote.times(new BigNumber(10).pow(18 - quoteTokenDecimals))
            lpTotalInQuoteToken = lpTotalInQuoteToken.times(new BigNumber(10).pow(18 - quoteTokenDecimals))
          }
        }else{
      ...
      const [info, totalAllocPoint, terraPerSec] = await multicall(masterchefABI, [
        {
      ...
      return {
        ...
        terraPerSec: new BigNumber(terraPerSec).toNumber(),
      }
    }),
  )
  return data
